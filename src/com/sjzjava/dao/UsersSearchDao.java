...
public class UsersSearchDao extends MybatisConnect {
    ...
    //部分匹配检索获取的用户列表
	public List<User> partSearchUsersSelect(int pageNumber,UsersSearchDto usersSearchDto) {		
		...
		List<User> listUsers = session.selectList(statement,usersSearchDto);
		return listUsers;
	}
    ...
    //完全匹配检索获取的用户列表
	public List<User> wholeSearchUsersSelect(int pageNumber,UsersSearchDto usersSearchDto) {		
		session = dbMybatisConnect();		
